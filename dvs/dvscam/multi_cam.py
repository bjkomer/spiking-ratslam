# can store and display multiple images at once
from . import dvscam
import numpy as np
import threading
...
class MultiCam(dvscam.NSTBot):
    def initialize(self):
        super(MultiCam, self).initialize()
        self.retina(False)
    ...
    def connect(self, connection):
        super(MultiCam, self).connect(connection)
    ...
    def disconnect(self):
        self.retina(False)
        super(MultiCam, self).disconnect()
    ...
    def show_images(self, decays=[0.5], display_mode='quick'):
        if len(self.images) == 0:
            for d in decays:
                self.images.append(np.zeros((128, 128), dtype=float))
            thread = threading.Thread(target=self.images_loop,
                                      args=(decays, display_mode))
            thread.daemon = True
            thread.start()
